...
| 保存形式／コンテンツ非対応 | ルーム復旧を停止し503を返す          |
| Queue送信前後のDO停止      | Storage outboxをalarmで再送          |
| Queue重複                  | `eventId`で無視                      |
| D1一時障害                 | Queue再試行                          |
| 古いクライアント           | 更新要求エラー                       |
...
- Queue受理済みの完了ルームは24時間後にalarmでスナップショットと一時状態を削除

Queue送信はat-least-onceとして扱います。`match_result_queue_retry_scheduled`の継続発生はoutbox滞留を示すためアラート対象とし、DLQの再投入時も同じ`eventId`を保持します。
