...
通常攻撃と敵の基本行動はサーバーが処理し、プレイヤーはスキル、ビルド、ルート、リスク選択など、通話中に相談したくなる判断へ集中します。

クラスは固定された役割ではなくビルドの土台として扱い、武器、装備、スキル、パッシブ、作戦によって実際の戦い方が決まります。画面はロビー、ビルド編集、戦闘、結果へ分離し、その時点で必要な情報と操作だけを表示する設計です。
...
ゲーム仕様書には完成時の製品仕様も含まれます。現在のコードは、自動戦闘、4クラス、スキル、3択強化、複数人ルーム、再接続、試合結果保存までを通した基盤実装です。ロードアウト編集、役割自動判定、状態別の画面分離などは目標仕様として文書化されています。
