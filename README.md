...
ゲーム仕様書には完成時の製品仕様も含まれます。現在のコードは、自動戦闘、4クラス、手動スキル、3択強化、複数人ルーム、再接続、試合結果保存までを通した基盤実装です。自由なロードアウト、自動戦闘作戦、遠征Lv.の自動強化、進行方針による自動ルート選択、状態別の画面分離などは目標仕様として文書化されています。
